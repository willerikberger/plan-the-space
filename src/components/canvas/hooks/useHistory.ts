...
export function useHistory({ getFabricState, restoreFromSnapshot }: UseHistoryOptions) {
  ...
  const captureSnapshot = useCallback(async () => {
    ...
    if (store.backgroundImageData) {
      backgroundImageRef = await manager.registerImage(store.backgroundImageData)
    }

    // Handle image objects — register their imageData and store ref
    const clonedObjects: PlannerObject[] = objects.map((obj) => {
      if ((obj.type === 'overlayImage' || obj.type === 'backgroundImage') && 'imageData' in obj) {
